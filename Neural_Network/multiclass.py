...
class MulticlassClassifier(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, num_hidden_units=10, step_size=.005, init_param_scale=0.01, max_num_epochs = 1000, num_class=3):
        ...
        self.affine = nodes.AffineNode( W=self.W1,x = self.x, b=self.b1,
                                                 node_name="affine")
        self.tanh = nodes.TanhNode(x = self.affine, node_name= "tanh")

        self.affine_n = nodes.AffineNode( W=self.W2,x = self.tanh, b=self.b2,
                                                 node_name="affine_n")
        self.softmax = nodes.SoftmaxNode(z=self.affine_n, node_name="soft")

        self.objective = nodes.NLLNode(y_hat=self.softmax, y=self.y,
                                               node_name="NLL")
        self.inputs = [self.x]
