...
class L2NormPenaltyNode(object):
    ...
    def backward(self):
        ## Your code
        # Calculate Partial Derivative w.r.t each parameter
...
class SoftmaxNode(object):
    ...
    def backward(self):
        d_z = self.d_out @ (self.out.reshape(-1,1) * np.identity(self.out.size)- self.out.reshape(-1,1)@ self.out.reshape(-1,1).T)
        self.z.d_out += d_z
        return self.d_out
...
class NLLNode(object):
    ...
    """
    def __init__(self, y_hat, y, node_name):
        self.y_hat = y_hat
        self.y = y
        self.node_name = node_name
        self.out = None
    ...
    def forward(self):
        self.out = -np.sum((self.y.out*np.log(self.y_hat.out)+ (1-self.y.out)*np.log(self.y_hat.out)))
        self.d_out = np.zeros(self.out.shape)
        return self.out

    def backward(self):
        d_y = -np.sum(np.log(self.y_hat.out))
        d_y_hat = self.d_out*(-(self.y.out/self.y_hat.out)+(1-self.y.out)/(1-self.y_hat.out))
        self.y.d_out += d_y
